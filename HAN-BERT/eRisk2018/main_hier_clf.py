...
def main(args):
    ...
    data_module = SymptomHierDataModule(args.bs, args.input_dir, tokenizer, args.max_len) #data_module 是用于加载和处理数据的类
    
    # 定义了一个 EarlyStopping 回调函数,用于在训练过程中进行早期停止."max",即当监测指标f1增大时,应停止训练。
    ...
    # 是一个回调函数，它可以帮助在训练过程中自动保存最佳模型权重，它会在每个训练 epoch 结束时，根据 val_f1 指标的表现保存最好的模型权重。mode 参数指定了最佳模型权重的评估标准，此处设置为 max,表示监测指标越大越好
...
if __name__ == "__main__":
    ...
    #   prajjwal1/bert-tiny
    parser.add_argument("--model_type", type=str, default="bert-base-uncased") #规模更小、参数更少的bert模型
    ...
    parser.add_argument("--input_dir", type=str, default="/E22301339/HAN-BERT/eRisk2018/processed/combined_maxsim32_gpt")
