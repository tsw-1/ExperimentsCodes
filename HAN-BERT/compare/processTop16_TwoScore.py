...
# 定义一个排序函数，根据帖子的第一个 key 进行升序排序
def sort_by_first_key(post):
    # 获取每个帖子的 keys，并将第一个 key 转换为整数
    first_key = int(list(post.keys())[0])
    return first_key
...
for folder in folders:
    ...
    for filename in natsorted(os.listdir(folder_path)):
        if filename.endswith('.json'):
            ...
            # 对前16条帖子根据第一个 key 进行升序排序
            top_16_posts = sorted(top_16_posts, key=sort_by_first_key)
