...
class  SymptomHierDataset(Dataset):
    def __init__(self, input_dir, tokenizer, max_len, split="train", max_posts=64):
        ...
        for fname in tqdm(os.listdir(input_dir2)):
            ...
            split_sym=split+"_Symptoms"
            input_dir2_sym=os.path.join(input_dir, split_sym)
            fname_sym=f"{fname[:-4]}_Symptoms.txt"
            symptoms=open(os.path.join(input_dir2_sym, fname_sym), encoding="utf-8").read().strip().split("\n")[:max_posts]

            # tokenizer接受一个帖子列表posts，将其编码为一个字典，其中包括输入ids、注意力掩码、token类型ids等信息
            tokenized_posts = tokenizer(posts, truncation=True, padding='max_length', max_length=max_len)
            sample_post={}
            for k, v in tokenized_posts.items():
                    sample_post[k] = v
             # tokenizer接受一个症状列表posts，将其编码为一个字典，其中包括输入ids、注意力掩码、token类型ids等信息
            tokenized_symptom = tokenizer(symptoms, truncation=True, padding='max_length', max_length=max_len)
            sample_symptom={}
            for k, v in tokenized_symptom.items():
                    sample_symptom[k] = v
            Sample["posts"]=sample_post
            Sample["symptoms"]=sample_symptom
            self.data.append(Sample)
            self.labels.append(label)
...
def my_collate_hier(data):
    ...
    for item, label in data:
        ...
        user_feats_posts={}
        user_feats_symptoms={}
        for k ,v in item["posts"].items():
            user_feats_posts[k]=torch.LongTensor(v)
        
        for k, v in item["symptoms"].items():
                    user_feats_symptoms[k]=torch.LongTensor(v)

        user_feats['posts']=user_feats_posts
        user_feats['symptoms']=user_feats_symptoms
...
'''
定义了一个 PyTorch Lightning 的 DataModule，它被用来加载训练和测试数据集。初始化时，它接受四个参数: batch size批大小, input_dir数据集的路径 ,tokenizer对象, max_len 表示序列的最大长度
'''
class SymptomHierDataModule(pl.LightningDataModule):
    def __init__(self, bs, input_dir, tokenizer, max_len):
        super().__init__()
        self.bs = bs
        self.input_dir = input_dir
    ...
    def setup(self, stage):
        if stage == "fit":
            self.train_set = SymptomHierDataset(self.input_dir, self.tokenizer, self.max_len, "train")
            self.val_set = SymptomHierDataset(self.input_dir, self.tokenizer, self.max_len, "test")
        
        elif stage == "test":
            self.test_set = SymptomHierDataset(self.input_dir, self.tokenizer, self.max_len, "test")
